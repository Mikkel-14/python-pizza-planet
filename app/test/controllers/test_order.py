import pytest
from app.controllers import IngredientController, OrderController, SizeController, BeverageController
...
def __order(ingredients: list, beverages: list, size: dict, client_data: dict):
    ...
    return {**client_data, "ingredients": ingredients, "size_id": size_id, "beverages": beverages}
...
def test_create(app, ingredients, size, client_data, beverages):
    ...
    beverages_in_created_order = set(item["_id"] for item in created_order["beverages"])
    pytest.assume(not beverages_in_created_order.difference(beverage_ids))
